...
$(document).ready(function()
{
	...
	var redirectTo = (targetPageName) => {
		return () => {
			if(currentPageName !== targetPageName)
			{
				...
				$("#puzzlesPage").removeClass("currentPage");
				$(targetPageName + "Page").attr("class", "currentPage");
				
				var previousPage = currentPageName;
				currentPageName = targetPageName;
				$(previousPage).fadeOut(500, function()
				{
					$(targetPageName).fadeIn(500);
	...
	$("#homePage, #logo").click(redirectTo("#home"));
	$("#homePage, #logo2").click(redirectTo("#home"));
	
	$("#aboutPage").click(redirectTo("#about"));
	$("#eventsPage").click(redirectTo("#events"));
	$("#puzzlesPage").click(redirectTo("#puzzles"));
	...
	// Scrolling images
	[...document.querySelectorAll('.single-row')].map(row => {
		row.style.setProperty('--animation', 'slide');
		row.style.setProperty('width', '200%');
